...
BEGIN_VS_SHADER( UnlitGeneric, "Help for UnlitGeneric" )
	...
	SHADER_DRAW
	{
		...
		if ( ( pShaderShadow == NULL ) && ( pShaderAPI != NULL ) && (params[RECEIVEFLASHLIGHT]->GetIntValue() == 0) && ShaderApiFast( pShaderAPI )->InFlashlightMode() ) // Not snapshotting && flashlight pass
		{
