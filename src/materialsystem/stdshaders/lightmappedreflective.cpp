...
BEGIN_VS_SHADER( LightmappedReflective_DX90, "Help for Lightmapped Reflective" )
	...
	SHADER_INIT
	{
		...
		{
			LoadTexture( REFRACTTEXTURE, g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER || false ? TEXTUREFLAGS_SRGB : 0 );
		}
		if ( params[REFLECTTEXTURE]->IsDefined() )
		{
			LoadTexture( REFLECTTEXTURE, g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER || false ? TEXTUREFLAGS_SRGB : 0 );
		}
		...
		IShaderDynamicAPI *pShaderAPI, bool bReflection, bool bRefraction )
	{
		...
		SHADOW_STATE
		{
			...
			if( bRefraction )
			{
				...
				pShaderShadow->EnableSRGBRead( SHADER_SAMPLER0, g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER || false );
			...
			if( bReflection )
			{
				...
				pShaderShadow->EnableSRGBRead( SHADER_SAMPLER2, g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER || false );
		...
		DYNAMIC_STATE
		{
			TextureBindFlags_t nBindFlags = ( g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER || false ) ? TEXTURE_BINDFLAGS_SRGBREAD : TEXTURE_BINDFLAGS_NONE;
