...
class CFileAsyncDirectoryScanJob : public CFileAsyncJob
{
	...
	public:
		...
		{
			Assert( pSearchSpec != nullptr && V_strlen( pSearchSpec )  < MAX_PATH && pAddCallback != nullptr && pDoneCallback != nullptr );
...
{
	FileHandle_t hFile = OpenEx( pszFilename, ( bAppend ) ? "ab+" : "wb", 0, nullptr );
	if ( hFile )
	{
