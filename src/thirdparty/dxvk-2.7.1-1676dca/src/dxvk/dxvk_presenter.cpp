...
namespace dxvk {
  ...
  VkResult Presenter::acquireNextImage(PresenterSync& sync, Rc<DxvkImage>& image) {
    ...
    // [CS_PATCH] Accept VK_SUBOPTIMAL_KHR as success — on Wayland multi-monitor
    // the compositor signals suboptimal on every frame during window movement,
    // causing an infinite recreate loop. The engine handles resize via Reset().
  ...
  VkResult Presenter::presentImage(uint64_t frameId, const Rc<DxvkLatencyTracker>& tracker) {
    ...
    // [CS_PATCH] Only recreate on actual errors, not VK_SUBOPTIMAL_KHR.
    std::lock_guard lock(m_surfaceMutex);
