...
// Check if the element has the readonly attribute
static bool IsReadOnly(ElementFormControl* element)
{
...
void WidgetTextInput::ProcessEvent(Event& event)
{
	...
	const bool readonly = IsReadOnly(parent);
	...
	case EventId::Keydown:
	{
		...
		case Input::KI_X:
		{
			if (ctrl && selection_length > 0 && !readonly)
			{
				CopySelection();
				DeleteSelection();
				DispatchChangeEvent();
	...
	{
		// Only process the text if no modifier keys are pressed and not readonly.
		if (!readonly && event.GetParameter<int>("ctrl_key", 0) == 0 && event.GetParameter<int>("alt_key", 0) == 0 && event.GetParameter<int>("meta_key", 0) == 0)
		{
