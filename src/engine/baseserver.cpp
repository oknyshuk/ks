								...
								const byte *pbEncryptionKey, int nEncryptionKeyIndex )
{
	...
	client->Connect( pchClientConnectionName, nNextUserID, netchan,
		false,	// real client
		clientPlatform,
		(splitScreenClients.Count() > 0) ? &splitScreenClients[0]->convars.get() : nullptr ); // userinfo if supplied
	...
	for ( i = 0 ; i < m_Clients.Count() ; ++i )
	{
		...
		if ( netchan->IsTimedOut() )
		{
			if ( this == &sv )
			{
				...
				{
					serverGameDLL->OnEngineClientNetworkEvent( pGameClient->edict, pGameClient->m_SteamID.ConvertToUint64(), INetChannelInfo::k_ENetworkEventType_TimedOut, nullptr );
				}
			}
