...
//
bool __MsgFunc_MatchEndConditions( const ks::net::CCSUsrMsg_MatchEndConditions &msg );
bool __MsgFunc_DisconnectToLobby( const ks::net::CCSUsrMsg_DisconnectToLobby &msg );
bool __MsgFunc_WarmupHasEnded( const ks::net::CCSUsrMsg_WarmupHasEnded &msg );
bool __MsgFunc_ServerRankUpdate( const ks::net::CCSUsrMsg_ServerRankUpdate &msg );
bool __MsgFunc_ServerRankRevealAll( const ks::net::CCSUsrMsg_ServerRankRevealAll &msg );
bool __MsgFunc_ScoreLeaderboardData( const ks::net::CCSUsrMsg_ScoreLeaderboardData &msg );
bool __MsgFunc_GlowPropTurnOff( const ks::net::CCSUsrMsg_GlowPropTurnOff &msg );
bool __MsgFunc_XpUpdate( const ks::net::CCSUsrMsg_XpUpdate &msg );
bool __MsgFunc_QuestProgress( const ks::net::CCSUsrMsg_QuestProgress &msg );
bool __MsgFunc_PlayerDecalDigitalSignature( const ks::net::CCSUsrMsg_PlayerDecalDigitalSignature &msg );
//
// WARNING!! WARNING!! WARNING!! WARNING!! WARNING!! WARNING!! WARNING!! WARNING!!
//
...
bool MsgFunc_KillCam( const ks::net::CCSUsrMsg_KillCam &msg ) 
{
...
bool MsgFunc_DisplayInventory( const ks::net::CCSUsrMsg_DisplayInventory &msg )
{
	C_CSPlayer *pPlayer = ToCSPlayer( C_BasePlayer::GetLocalPlayer() );
	if ( !pPlayer )
		return true;
...
void ClientModeCSNormal::Init()
{
	...
	m_UMCMsgKillCam.Bind< ks::net::CS_UM_KillCam ,ks::net::CCSUsrMsg_KillCam >( UtlMakeDelegate ( MsgFunc_KillCam ) );

	if ( this != GetFullscreenClientMode() )
	{
		m_UMCMsgDisplayInventory.Bind< ks::net::CS_UM_DisplayInventory, ks::net::CCSUsrMsg_DisplayInventory>( UtlMakeDelegate(MsgFunc_DisplayInventory) );
	}
...
//			int iItemRarity = event->GetInt( "quality" );
			...
			if ( C_CS_PlayerResource *cs_PR = dynamic_cast<C_CS_PlayerResource *>( g_PR ) )
			{
				...
				wchar_t const *wszItemFound = pszLocString ? g_pLocalize->Find( pszLocString ) : nullptr;
				if ( wszItemFound )
				{
...
bool __MsgFunc_SendPlayerItemFound( const ks::net::CCSUsrMsg_SendPlayerItemFound &msg )
{
	...
	if ( C_CS_PlayerResource *cs_PR = dynamic_cast< C_CS_PlayerResource * >( g_PR ) )
	{
		...
		const char *pszLocString = g_pszItemFoundMethodStrings[ iMethod ];
		wchar_t *wszItemFound = pszLocString ? g_pLocalize->Find( pszLocString ) : nullptr;
		if ( wszItemFound )
		{
			wchar_t wszLocalizedString[ 2 * MAX_DECORATED_PLAYER_NAME_LENGTH ] = {};
			if ( wszLocalizedString[0] )
...
ks::net::ScoreLeaderboardData ClientModeCSNormal::s_ScoreLeaderboardData;

bool __MsgFunc_ScoreLeaderboardData( const ks::net::CCSUsrMsg_ScoreLeaderboardData &msg )
{
	ClientModeCSNormal::s_ScoreLeaderboardData = {};
	if ( msg.data.has_value() )
		ClientModeCSNormal::s_ScoreLeaderboardData = msg.data;
	return true;
}
...
bool __MsgFunc_PlayerDecalDigitalSignature( const ks::net::CCSUsrMsg_PlayerDecalDigitalSignature &msg )
{
	...
	if ( msg.data->startpos.size() != 3 ) return true;
	...
	if ( msg.data->endpos.size() != 3 ) return true;
...
void PlayerDecalDataSendActionSprayToServer( int nSlot )
{
	...
	ks::net::PlayerDecalDigitalSignature &dd = msg.data.mut();
	dd.accountid = s_mysteamid.GetAccountID();
	dd.equipslot = data.m_nEquipSlot;
	dd.tx_defidx = data.m_nDefIdx;
	dd.tint_id = data.m_unTintID;
	dd.creationtime = data.m_flCreationTime;
	dd.trace_id = nRandomKey;

	// The user message is ours now, so it goes straight into the envelope.
	CSVCMsg_UserMessage_t um;
	um.msg_type = ks::net::CS_UM_PlayerDecalDigitalSignature;
	um.msg_data.mut().resize( ks::proto::byte_size( msg ) );
	ks::proto::write( msg, ( std::byte * ) um.msg_data.mut().data() );
