...
static CResponseQueue::CFollowupTargetSpec_t ResolveFollowupTargetToEntity( AIConcept_t &conc, AI_CriteriaSet &criteria, const char * RESTRICT szTarget, AI_Response * RESTRICT response = NULL )
{
	...
	{
		return CResponseQueue::CFollowupTargetSpec_t( kDRT_SPECIFIC, conc.GetSpeaker() );
	}
	...
	{
		return CResponseQueue::CFollowupTargetSpec_t( kDRT_ANY, conc.GetSpeaker() );
	}
...
// TODO: Currently uses awful stricmp. Use symbols! Once I know which ones we want, that is.
static CResponseQueue::CFollowupTargetSpec_t ResolveFollowupTargetToEntity( AIConcept_t &conc, AI_CriteriaSet &criteria, AI_Response * RESTRICT response, AI_ResponseFollowup * RESTRICT followup )
{
...
// Input  : *response -
//-----------------------------------------------------------------------------
bool CAI_ExpresserWithFollowup::SpeakDispatchResponse( AIConcept_t &conc, AI_Response *response, AI_CriteriaSet *criteria, IRecipientFilter *filter )
{
	// This gives the chance for the other bot to respond.
	if ( !conc.GetSpeaker().IsValid() )
	{
		conc.SetSpeaker( GetOuter() );
	}

	bool bInterrupted = IsSpeaking();
	bool bSuc = CAI_Expresser::SpeakDispatchResponse( conc, response, criteria, filter );
	if (!bSuc)
	{
		return false;
	}
				 ...
				 followup->followup_delay >= 0 )
			{
				...
				if ( fTimeToLastSpeech > 0 )
				{
					DispatchFollowupThroughQueue( followup->followup_concept, followup->followup_contexts,
						ResolveFollowupTargetToEntity( conc, *criteria, response, followup ),
				...
				else  // error
				{
					...
					if ( criteria )
						m_followupTarget = ResolveFollowupTargetToEntity( conc, *criteria, response, m_pPostponedFollowup );
					else
					{
						AI_CriteriaSet tmpCriteria;
						m_followupTarget = ResolveFollowupTargetToEntity( conc, tmpCriteria, response, m_pPostponedFollowup );
					}
				}
			}
...
void CAI_ExpresserWithFollowup::DispatchFollowupThroughQueue( const AIConcept_t &conc,
...
void CC_RR_ForceConcept_f( const CCommand &args )
{
	...
	AIConcept_t conc( args[2] );
	QueueSpeak( conc, ResolveFollowupTargetToEntity( conc, criteria, args[1] ), criteria );
}
