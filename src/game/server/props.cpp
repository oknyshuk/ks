...
void CBreakableProp::AnimateThink( void )
{
	if ( m_nPhysgunState == PHYSGUN_ANIMATE_IS_PRE_ANIMATING || m_nPhysgunState == PHYSGUN_ANIMATE_IS_POST_ANIMATING )
	{
		...
		if ( IsActivityFinished() )
		{
			...
			SetContextThink( nullptr, 0, s_pPropAnimateThink );
		}
	}
}
...
{
	SetGlobalFadeScale( 0.0f );
	SetContextThink( nullptr, gpGlobals->curtime, s_pFadeScaleThink );
}
...
void CDynamicProp::InputSetGlowDisabled( inputdata_t &inputdata )
{
	...
	ks::net::CCSUsrMsg_GlowPropTurnOff msg;
	msg.entidx = entindex();	// this prop
	SendUserMessage( filter, ks::net::CS_UM_GlowPropTurnOff, msg );	
}
...
void CPhysicsProp::ClearFlagsThink( void )
{
	...
	if ( VPhysicsGetObject() )
	{
		...
		SetContextThink( nullptr, 0, "PROP_CLEARFLAGS" );
	}
}
...
{
	ErrorIfNot( GetModel() != nullptr, ( "prop_door with no model at %.2f %.2f %.2f\n", GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z ) );
...
{
	SetContextThink( nullptr, 0, "AreaPortal" );
