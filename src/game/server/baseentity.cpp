...
void CBaseEntity::PostConstructor( const char *szClassname )
{
	...
	Assert( m_iClassname != NULL_STRING && STRING(m_iClassname) != nullptr );
...
int CBaseEntity::VPhysicsTakeDamage( const CTakeDamageInfo &info )
{
	...
	Assert(VPhysicsGetObject() != nullptr);
	if ( VPhysicsGetObject() )
	{
...
void CBaseEntity::ShadowCastDistThink( )
{
	...
	SetContextThink( nullptr, gpGlobals->curtime, "ShadowCastDistThink" );
}
...
{ 
	...
	{
		SetContextThink( nullptr, TICK_NEVER_THINK, "FrictionRevertThink" );
	}
#endif
}
