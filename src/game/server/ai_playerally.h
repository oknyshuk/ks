...
public:
	...
	ConceptInfo_t *GetConceptInfo( AIConcept_t conc );
	void OnSpokeConcept( CAI_PlayerAlly *pPlayerAlly, AIConcept_t conc, AI_Response *response  );
	...
	bool ConceptDelayExpired( AIConcept_t conc );
...
public:
	...
	virtual void PostSpeakDispatchResponse( AIConcept_t conc, AI_Response *response );
	...
	bool 		SelectSpeechResponse( AIConcept_t conc, const char *pszModifiers, CBaseEntity *pTarget, AISpeechSelection_t *pSelection );
	void		SetPendingSpeech( AIConcept_t conc, AI_Response *pResponse );
	...
	bool		ShouldSpeakRandom( AIConcept_t conc, int iChance );
	bool		IsAllowedToSpeak( AIConcept_t conc, bool bRespondingToPlayer = false );
	virtual bool SpeakIfAllowed( AIConcept_t conc, const char *modifiers = nullptr, bool bRespondingToPlayer = false, char *pszOutResponseChosen = nullptr, size_t bufsize = 0 );
	...
	void		OnSpokeConcept( AIConcept_t conc, AI_Response *response );
