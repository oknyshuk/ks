...
static bool Helper_CheckFieldAppliesToTeam( char const *szField, int nTeam )
{
    ...
    float CCSGameRules::GetExplosionDamageAdjustment(Vector & vecSrc, Vector & vecEnd, CBaseEntity *pEntityToIgnore)
    {
        ...
        else if (!(tr.DidHitWorld()) && (tr.Ent<CBaseEntity>() != nullptr) && (tr.Ent<CBaseEntity>() != pEntityToIgnore) && (tr.Ent<CBaseEntity>()->GetOwnerEntity() != pEntityToIgnore))
        {
				...
				case k_EQueuedMatchmakingRematchState_VoteStarting:
					{
						...
						if ( m_pQueuedMatchmakingReportedRoundStats )
						{
							...
							if ( numHumansPresent == m_numQueuedMatchmakingAccounts )
								szCannotRematchExplanation = nullptr;
						}
...
// 			// disable the spawn after someone spawn here in coop
    ...
    bool CCSGameRules::IsThereABomb()
    {
        ...
        if( gEntList.FindEntityByClassname( nullptr, WEAPON_C4_CLASSNAME ) != 0 )
        {
            bBombFound = true;
        }
        /* what about planted bombs!? */
        else if( gEntList.FindEntityByClassname( nullptr, PLANTED_C4_CLASSNAME ) != 0 )
        {
            bBombFound = true;
        }
...
{
	Assert(pListener != nullptr && pSpeaker != nullptr);
...
void CCSGameRules::CloseBuyMenu( int nPlayerID )
{
	...
	C_WeaponCSBase *pWeapon = (pPlayer && pPlayer->IsLocalPlayer()) ? pPlayer->GetActiveCSWeapon() : nullptr ;
	if ( pWeapon )
	{
